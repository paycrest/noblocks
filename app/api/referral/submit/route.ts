    ...
    try {
        ...
        if (!/^NB[A-Z0-9]{4}$/.test(normalizedCode)) {
            return NextResponse.json(
                {
                    success: false,
                    error: "Invalid referral code format. Use 6 characters: NB plus 4 letters or numbers (e.g. NB738K).",
        ...
        if (referrer.wallet_address.toLowerCase() === walletAddress) {
            return NextResponse.json(
                {
                    success: false,
                    error: "You cannot use your own referral code. Ask a friend for theirs.",
                    code: "SELF_REFERRAL",
                },
                { status: 400 }
            );
