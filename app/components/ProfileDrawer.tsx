...
export default function ProfileDrawer({ isOpen, onClose }: ProfileDrawerProps) {
    ...
    const monthlyProgress = (transactionSummary.monthlySpent / currentLimits.monthly) * 100;
    ...
    const walletAddress = user?.linkedAccounts.find((account) => account.type === "smart_wallet")?.address;
        ...
        <div className="space-y-4 animate-pulse">
            ...
            <div className="space-y-4 rounded-[20px] border border-border-light bg-transparent p-4 dark:border-white/5">
                ...
                <div className="flex items-center gap-4">
                    <div className="w-[44px] h-[44px] rounded-full bg-gray-200 dark:bg-white/10"></div>
            ...
            <div className="space-y-4 rounded-[20px] border border-border-light bg-transparent p-4 dark:border-white/5">
                ...
                <div className="space-y-3">
                    ...
                    <div className="h-8 w-full bg-gray-200 dark:bg-white/10 rounded"></div>
                    <div className="h-2 w-full bg-gray-200 dark:bg-white/10 rounded-full"></div>
                </div>
                <div className="h-11 w-full bg-gray-200 dark:bg-white/10 rounded-xl"></div>
            </div>

            {/* Tier sections skeleton */}
            {[1, 2].map((i) => (
                <div key={i} className="rounded-[20px] border border-border-light bg-transparent p-4 dark:border-white/5">
    ...
    const renderTierSection = (tierLevel: number) => {
        ...
        return (
            <div key={tierLevel} className="border-b border-border-light dark:border-white/10 last:border-b-0">
                <button
                    type="button"
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-light text-text-secondary dark:text-lavender-500">{isExpanded ? "See less" : "See more"}</span>
                        <ArrowDown01Icon
                            className={classNames(
                                "size-4 text-outline-gray dark:text-lavender-500 transition-transform",
                                isExpanded ? "" : ""
                            )}
                        />
                    </div>
                            ...
                            <div className="px-4 pb-4 space-y-4">
                                <ul className="space-y-4">
                                    {tierData.requirements.map((req, index) => (
                                        <li key={index} className="flex items-center gap-2 text-xs text-text-secondary dark:text-white/60">
                                            <div className='p-0.5 bg-white/10 rounded-lg'>
                                                {req.includes('number') ? (
                                ...
                                <div className="flex justify-between dark:bg-white/5 px-4 py-3 rounded-2xl">
                                    <p className='text-sm text-text-secondary dark:text-white/60'>Limit</p>
                                    <p className="font-light text-xs text-text-body dark:text-white/80"><span className='text-sm font-medium'>${formatNumberWithCommas(tierData.limits.monthly)}</span> / month</p>
                                </div>
