...
export function TransactionStatus({
  ...
  useEffect(() => {
    ...
    if (!orderId) return;
    if (forwardTriggeredOrderRef.current === orderId) return;
    // EVM-only for now; Starknet forwarding isn't supported by this flow.
    if (selectedNetwork.chain.name === "Starknet") return;

    const destination = recipientWalletAddress.trim();
    if (!destination) return;

    const noblocksWallet = embeddedWallet?.address ?? "";
    // Self short-circuit: destination is the Noblocks wallet — funds already at destination.
    if (
      noblocksWallet &&
      destination.toLowerCase() === noblocksWallet.toLowerCase()
    ) {
      forwardTriggeredOrderRef.current = orderId;
    ...
    const match = tokens.find(
      (t) => t.symbol.toUpperCase() === String(token).toUpperCase(),
    );
    if (!match || !(amountReceivedCrypto > 0)) return;
  ...
  // Reflect the outcome of the leg-2 transfer (the hook reports success/error asynchronously).
  useEffect(() => {
  ...
  const getPaymentMessage = () => {
    ...
    const amountDisplay = isOnramp
      ? `${formatCurrency(Number(amount) || 0, currency, `en-${currency.slice(0, 2)}`)} (${token} ${formatNumberWithCommas(fiat)})`
      : `${formatNumberWithCommas(amount)} ${token} (${formatCurrency(fiat ?? 0, currency, `en-${currency.slice(0, 2)}`)})`;
        ...
        <div className="flex w-fit flex-col items-end gap-2 text-neutral-900 dark:text-white/80">
          ...
          >
            ...
            <p className="whitespace-nowrap pr-4 font-medium">
              {isOnramp
                ? formatCurrency(Number(amount) || 0, currency, `en-${currency.slice(0, 2)}`)
          ...
          <AnimatedComponent
            variant={slideInOut}
            delay={0.4}
            className="max-w-60 truncate whitespace-nowrap rounded-full bg-gray-50 px-3 py-1 dark:bg-white/5"
          >
            {isOnramp
      ...
      <div className="flex flex-col items-start gap-4 sm:max-w-xs">
        ...
        <div className="flex w-full items-center gap-2 text-neutral-900 dark:text-white/80 sm:hidden">
          ...
          <AnimatedComponent
            variant={slideInOut}
            delay={0.4}
            className="max-w-28 truncate rounded-full bg-gray-50 px-3 py-1 dark:bg-white/5"
          >
            {isOnramp
              ? (recipientWalletAddress ? `${recipientWalletAddress.slice(0, 6)}...${recipientWalletAddress.slice(-4)}` : "")
              : formatRecipientNameFirstWordForPill(recipientName)}
