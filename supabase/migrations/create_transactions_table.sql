...
-- Grant permissions
GRANT
EXECUTE ON FUNCTION set_current_wallet_address (TEXT) TO service_role;

GRANT
EXECUTE ON FUNCTION set_current_wallet_address (TEXT) TO authenticated;
...
SELECT USING (
        wallet_address = current_setting(
            'app.current_wallet_address', TRUE
        )
    );
